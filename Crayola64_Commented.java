...
public class Crayola64_Commented {

   public static final boolean WHITEOUTLINE = false;	//whether or not there is an outline around the blank color used for space.
   public static final int FONTSIZE = 100;	//pixels
   public static final int COLUMNWIDTH = 6;	//how many characters wide the screen can be.
   public static final int YMARGIN = 20;	//pixels
   public static final int XMARGIN = 20;	//pixels
	...
	public static void processor(String message) throws FileNotFoundException  {
      ...
      for (int i = 0; i < message.length(); i++) {
         char ch = message.charAt(i);
			...
			//handle capitals and <{[(/ characters differently by preceding with a '`'.
         if (ch == 40 || ch == 60 || ch == 47 || ch == 91 || ch == 123 || (ch > 64 && ch < 91)) {
            converter(g, '`', x, y);
            if (ch > 64 && ch < 91) {
               ch = (char) (ch + 32);//now treat the capital letter like lowercase.
            }//eoif
            //advance the x-marker and go to the next line if necessary.
				x += FONTSIZE;
   ...
   public static int panelY(String message) {
	...
	//because some characters require two colors to represent them,
	//the length of the typed message might be different from the output length.
